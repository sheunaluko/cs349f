...
# MOMENTUM
class PeriodicTraderStrategy(AlgorithmicTrader):
    ...
    def algorithm(self, df, **kwargs):
        ...
        y_timeseries = df['ClosePrice']
        # smooth timeseries
        box_pts, ma_length = 10, 10
        box = np.ones(box_pts)/box_pts
        y_smooth = np.convolve(y_timeseries, box, mode='same')
        
        dydx = np.diff(y_smooth)/np.diff(range(len(y_smooth)))
        dydx = np.insert(dydx, 0, 0.0, axis=0)
    
        sign_dy_current = math.copysign(1, np.mean(dydx[-ma_length:]))
        sign_dy_prev = math.copysign(1, np.mean(dydx[-2*ma_length: -ma_length]))
